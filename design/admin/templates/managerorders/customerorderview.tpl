...
<br/><br/>
<table border="0">
{foreach $user_content.data_map as $item}
	<tr>
		{if $item.has_content}
			{if eq($item.contentclass_attribute_name, 'User account')}
				<td>					
					<b>Email</b>
				</td>
				<td>				
			...
			{else}
				<td>					
					<b>{$item.contentclass_attribute_name}</b>
				</td>
				<td>				
...
<br/>
<a href={$user_content.main_node.url_alias|ezurl()}>Go to the user</a>
...
{section var=Orders loop=$order_list sequence=array( bglight, bgdark ) max=$limit offset=$view_parameters.offset}
...
<tr class="{$Orders.sequence}">
	<td><a href={concat( '/managerorders/orderview/', $Orders.item.id, '/' , 'D' )|ezurl}>{$Orders.item.order_nr}</a></td>
...
{include name=navigator
         ...
         page_uri=concat('/managerorders/customerorderview/',$customer_id)
         item_count=$order_list|count
